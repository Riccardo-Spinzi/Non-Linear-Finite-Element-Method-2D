function MODEL = stress_recovery( MODEL, SOL, MATERIAL, ELEMENT )

         % % --------------- FUNCTION INFO ---------------- % %

% stress_recovery calculates the stresses present in the structure (only
% for the linear case) and manipulates them to input them to the
% stress_plot.m function.
%
...
for i = 1 : MODEL.nels
    ...
    for iG = 1 : nint_p
        ...
        for jG = 1 : nint_p
            ...
            % --- Get displacements at useful nodes
            U = MODEL.UxUy(nodes,:);
            [r,c] = size(U);
            U_calc = reshape(U',r*c,1);

            % --- Get stress at the integration points
